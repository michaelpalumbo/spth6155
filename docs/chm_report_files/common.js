...
function common() {
    return {
        ...
        combineDataSources: function(dataSources) {
            var it = {};
            var notifyListeners = observable(it);

            var firstObject = dataSources[0];
            var functionNames = _.filter(_.functions(firstObject), function(name){
                // exclude functions below so that update listeners are only registered on combined data source
                return !_.contains(["onUpdate", "sendUpdate"], name);
            });
            var attributeNames = _.filter(_.keys(firstObject), function(name){
                return !_.contains(functionNames, name);
            });
            attributeNames.forEach(function(name) {
                it[name] = firstObject[name];
            });
            functionNames.forEach(function(name) {
                it[name] = function(args) {
                    dataSources.forEach(function(dataSource){ dataSource[name](args); });
                };
            });

            var updates = [];
            dataSources.forEach(function(o) {
                o.onUpdate(function(update){ updates.push(update); });
            });
            it.sendUpdate = function() {
                updates = [];
                dataSources.forEach(function(o){ o.sendUpdate(); });

                var keys = _.uniq(_.union(
                    _.flatten(_.map(_.pluck(updates, "min"), function(m) { return _.keys(m); })),
                    _.flatten(_.map(_.pluck(updates, "max"), function(m) { return _.keys(m); }))
                ));
                keys.forEach(function(key) {
                    var minMin = _.min(_.pluck(updates, "min"), function(m) { return m[key]; })[key];
                    var maxMax = _.max(_.pluck(updates, "max"), function(m) { return m[key]; })[key];
                    updates.forEach(function(update) {
                        update.min[key] = minMin;
                        update.max[key] = maxMax;
                    });
                });
